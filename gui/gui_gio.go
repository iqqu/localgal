...
func handleEvent(e event.Event, mw *MainWindow) {
	...
	case app.FrameEvent:
		...
		layout.Inset{Top: unit.Dp(4), Right: unit.Dp(8), Bottom: unit.Dp(8), Left: unit.Dp(8)}.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
			return layout.Flex{Axis: layout.Vertical}.Layout(gtx,
				...
				layout.Rigid(func(gtx layout.Context) layout.Dimensions {
					if mw.cwd != "" {
						in := layout.Inset{Top: unit.Dp(-10)}
						return in.Layout(gtx, material.Body2(mw.th, fmt.Sprintf("Paths are relative to:\n%s", mw.cwd)).Layout)
					}
				...
				layout.Rigid(func(gtx layout.Context) layout.Dimensions {
					...
					return layout.Flex{Axis: layout.Vertical}.Layout(gtx,
						...
						layout.Rigid(labeledEditor(mw.th, labelWidth, vars.EnvDflogRoot.Key(), &mw.dflogRootEd, fmt.Sprintf("Base directory to resolve relative paths in %s from", vars.EnvDflog.Key()))),
					)
				}),
				layout.Rigid(func(gtx layout.Context) layout.Dimensions {
					// Buttons
							...
							return layout.Flex{Axis: layout.Horizontal}.Layout(gtx,
								layout.Flexed(1, func(gtx layout.Context) layout.Dimensions {
									...
									return lst.Layout(gtx, 1, func(gtx layout.Context, _ int) layout.Dimensions {
										...
										editor.TextSize = unit.Sp(10)
										editor.Font.Typeface = "monospace"
										editor.Font.Weight = font.SemiBold
...
func getLabelMaxWidth(gtx layout.Context, mw *MainWindow, labels []string) int {
	transparent := color.NRGBA{A: 0}
	var childs []layout.FlexChild
	for _, lbl := range labels {
		childs = append(childs, layout.Rigid(func(gtx layout.Context) layout.Dimensions {
			lbl := InputTitle(mw.th, lbl)
			lbl.Color = transparent
			return lbl.Layout(gtx)
		}),
		)
	}
	//keyDim := layout.Flex{Axis: layout.Vertical}.Layout(gtx, keys...)
	inKD := 0
	_ = layout.Flex{}.Layout(gtx,
		layout.Rigid(func(gtx layout.Context) layout.Dimensions {
			dimensions := layout.Flex{Axis: layout.Vertical}.Layout(gtx,
				childs...,
			)
			inKD = dimensions.Size.X
			return dimensions
		}),
...
func InputTitle(th *material.Theme, label string) material.LabelStyle {
	return material.Label(th, th.TextSize*13/16, label)
}

func InputCaption(th *material.Theme, label string) material.LabelStyle {
	return material.Label(th, th.TextSize*11/16, label)
}
			...
			return layout.Flex{Axis: layout.Vertical, Spacing: layout.SpaceEnd}.Layout(gtx,
				layout.Rigid(func(gtx layout.Context) layout.Dimensions {
					return layout.Flex{Alignment: layout.Middle}.Layout(gtx,
						layout.Rigid(func(gtx layout.Context) layout.Dimensions {
							lbl := InputTitle(theme, label)
							gtx.Constraints.Min.X = gtx.Dp(gtx.Metric.PxToDp(lblWidth))
							return lbl.Layout(gtx)
						}),
						layout.Flexed(1, func(gtx layout.Context) layout.Dimensions {
							border := widget.Border{Color: color.NRGBA{R: 0xc8, G: 0xc8, B: 0xc8, A: 0xff}, Width: unit.Dp(1), CornerRadius: unit.Dp(4)}
							pad := layout.Inset{Left: unit.Dp(6), Right: unit.Dp(6)}
							in := layout.Inset{Left: unit.Dp(6)}
							return in.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
								return border.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
									editor := material.Editor(theme, ed, "")
									//editor.Font.Typeface = "monospace"
									editor.TextSize *= 14.0 / 16.0
									return pad.Layout(gtx, editor.Layout)
								})
							})
						}))
				}),
				layout.Rigid(func(gtx layout.Context) layout.Dimensions {
					hel := InputTitle(theme, help)
					col := theme.Palette.Fg
					col.A = 0xee
					hel.Color = col
					in := layout.Inset{Left: unit.Dp(6), Right: unit.Dp(6)}
...
//func labeledEditor(theme *material.Theme, lblWidth unit.Dp, label string, ed *widget.Editor, help string) func(gtx layout.Context) layout.Dimensions {
//	return func(gtx layout.Context) layout.Dimensions {
//		return layout.Flex{Axis: layout.Vertical}.Layout(gtx,
//			layout.Rigid(func(gtx layout.Context) layout.Dimensions {
//				lbl := InputTitle(theme, label)
